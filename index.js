...
const createColorPopup = (color) => {
  ...
  const colorValus = popup.querySelectorAll(".value");
  colorValus.forEach((value) => {
...
const showColors = () => {
  colorList.innerHTML = pickerColors
    .map(
      (color) =>
        `
        <li class="color">
  ...
  const pickerColorsContainer = document.querySelector(".colors-list");
  pickerColorsContainer.classList.toggle("hide", pickedColors.length === 0);
};
